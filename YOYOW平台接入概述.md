...
以上各组密钥，由一对公私钥对组成，公钥为以YYW开头的53个字符，私钥为51个字符。可以通过网页钱包查看各个密钥（详见：yoyow keys tutorial. yoyow私钥教程）。比如：
...
## 接入流程：![](./images/join_up_process.png)
...
注：1、SDK 需要配置零钱权限
2、考虑到安全问题，SDK 强烈建议单独部署于内网机器中，不对外开放IP与端口。
...
在YOYOW的SDK中提供了transfer操作，可以转出自己的资产（YOYO或者自己创建的资产）分发给平台用户。详见：YOYOW Middleware #1.3. 转账到指定用户 transfer （需要安全验证的请求）  
...
在YOYOW的SDK中提供了transferFromUser，可以支持用户对用户通过平台转账（只可以转出零钱内存放的YOYO代理），同时将转账数据写入YOYOW链上。详见：YOYOW Middleware #1.9. 用户对用户通过平台转账 transferFromUser（需要安全验证的请求）
...
注：1. 在YOYOW客户端的使用中，根据需求，需要导入相应的密钥，详见：《交易所对接指南：启动YOYOWClient》
2. 在YOYOW SDK的使用中，也需要在配置文件中根据需要填写相应的资金密钥，零钱密钥和备注密钥，如果使用不到，尽量不填写。


[2]:	file:///./images/join_up_process.png "接入流程图"
